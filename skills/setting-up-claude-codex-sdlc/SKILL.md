...
1. **Check the machine** and report anything missing. Don't install it yourself.
   ...
   - **Pushing workflow files.** The requirement depends on which credential actually authenticates the push, not just the remote's transport. Get the origin URL (`git remote get-url origin`), then check `git config --get-urlmatch credential.helper <that URL>` — the plain `credential.helper` key doesn't reflect this: `gh auth setup-git` writes a URL-scoped section (`[credential "https://github.com"] helper = !'<absolute path to gh>' auth git-credential`), not the bare, unscoped string a naive check might expect:
     - **The urlmatch result contains `auth git-credential`** (however `gh auth setup-git` quoted or pathed it): `gh`'s token is the real push credential, so `gh auth status` should list the `workflow` scope. If it's missing, the user runs `gh auth refresh -h github.com -s workflow` **in their own terminal**; inside Claude Code the device code expires unseen.
     - **Anything else** (Git Credential Manager, the macOS Keychain, a manually configured PAT, SSH, or a fine-grained token in `GH_TOKEN`): `gh auth status` doesn't necessarily reflect the credential `git push` actually uses, so don't flag it from that alone. Treat the first push that adds a workflow file as the real test: a rejection like `refusing to allow an OAuth App to create or update workflow` means that credential needs the `workflow` scope added at its own source (the credential manager, Keychain entry, or the PAT's own settings), not via `gh auth refresh`.
