...
1. `git fetch --prune`. List the merged PRs and the ones still open. Follow `docs/DEVELOPMENT-PROCESS.md` → **Stacked branches: drain before merging**: audit merged stack bases for stranded work, including deeper layers, and confirm each open PR targets a surviving base. Preserve local refs/worktrees until this audit is complete; recover missing work through a new PR, never an automatic merge.
...
4. Remove the worktrees under `.superpowers/` whose work is verified to have reached the surviving base in step 1. A merged PR alone is insufficient, especially after squash/rebase merges or late child merges. First check each one with `git status`, and report anything uncommitted or unaccounted for instead of deleting it.
